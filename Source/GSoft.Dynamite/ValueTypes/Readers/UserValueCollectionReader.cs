...
{
    /// <summary>
    /// Reads UserMulti-based field values
    /// </summary>
    public class UserValueCollectionReader : BaseValueReader<UserValueCollection>
    {
        /// <summary>
        ...
        public override UserValueCollection ReadValueFromListItem(SPListItem item, string fieldInternalName)
        {
            var fieldValue = item[fieldInternalName];

            if (fieldValue != null)
            {
                var userFieldVal = (SPFieldUserValueCollection)fieldValue;
                return new UserValueCollection(userFieldVal);
            }
        ...
        public override UserValueCollection ReadValueFromListItemVersion(SPListItemVersion itemVersion, string fieldInternalName)
        {
        ...
        public override UserValueCollection ReadValueFromCamlResultDataRow(SPWeb web, DataRow dataRowFromCamlResult, string fieldInternalName)
        {
            var message = string.Format(
                CultureInfo.InvariantCulture,
                "Cannot read full UserValueCollection information when it has been converted to a data cell (fieldName={0}).",
                fieldInternalName);
